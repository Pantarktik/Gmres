	program poisson3d
	implicit none
	integer, parameter:: nr=10,nteta=10,nphi=10
	...
	real*8 rho(nrow),out_bound(nteta,nphi),var(nnzero),v(nrow),mus,y(nrow),x0(nrow)
	real*8 teta(nteta),dteta,tetapop(nteta+1),teta_bound(nr),teta_niz,teta_verh
	real*8 r1(nrow),teta1(nrow),phi1(nrow),r_out1,rho0,lambda,I11
	...
	real*8 a1 !HORIZONTAL POLUOS
	real*8 a2 !VERTICAL POLUOS	
...
!	open(1,file='rtp')
	...
	do i=1,nrow
	r1(i)=r( (i-1)/(nteta*nphi)+1 )
	teta1(i)= teta( (mod(i,nteta*nphi)-1)/nphi+1 )
	phi1(i)= phi(mod(mod(irow,(nteta*nphi))-1,nphi)+1)
!	write(1,*)  (i-1)/(nteta*nphi)+1,(mod(i-1,nteta*nphi))/nphi+1,mod(mod(i-1,(nteta*nphi)),nphi)+1
	enddo
...
!	stop
	...
	do irow=1,nrow
		if( &
		( (r1(irow)*cos(teta1(irow)))**2/a1**2+(r1(irow)*sin(teta1(irow)))**2/a2**2 ) &
		.le.1.)then
			rho(irow)=rho0
		else
			rho(irow)=0.
	...
	do j=1,nteta
		lambda=0.5*( r_out1**2-a1**2-a2**2+ &
		sqrt( (a1**2+a2**2-r_out1**2)**2-4*(a1**2*a2**2-r_out1**2*(cos(teta(j))**2*a2**2+sin(teta(j))**2*a1**2)) ) &
		 )
		 
		I11=pi/sqrt(a1**2-a2**2)-2./sqrt(a1**2-a2**2)*atan(sqrt( (a2**2+lambda)/(a1**2-a2**2) ))
		
		out_bound(j,:)=pi*g*rho0*a1**2*a2*(                                         &
		I11*(1+r_out1**2/(a2**2-a1**2)*(cos(teta(j))**2/2.-sin(teta(j))**2) )       &
		-(r_out1*cos(teta(j)))**2*sqrt(a2**2+lambda)/(a2**2-a1**2)/(a1**2+lambda)   &
		-(r_out1*sin(teta(j)))**2*(2./(a1**2+lambda)/sqrt(a2**2+lambda)-2.*sqrt(a2**2+lambda)/(a2**2-a1**2)/(a1**2+lambda) ) &
		)
	enddo
	...
	do j=1,nteta
	do k=1,nphi
		write(1,*) teta(j),phi(k),out_bound(j,k)
	enddo
	enddo
	close(1)
!	stop
...
!	a(i,i)=-(rpop(i)**2+rpop(i+1)**2)/dr   !!!AA
	...
	if(i.eq.nr)then
	rhs(irow)=rhs(irow)-out_bound(j,k)*rpop(nr+1)**2/dr*dphi	
	endif
	if(j.eq.1)then
...
!	close(1)
	...
	enddo
	write(1,*) r(nr)+dr,out_bound(1,1)
	close(1)
	
	open(1,file='3dteta.dat')
	do i=1,nrow
		if((i/(nteta*nphi)+1).eq.5)then
			write(1,*) teta1(i),x(i)
		endif
	enddo
	close(1)	
